<a {{ $attributes }} class="bg-white h-fit w-full p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300" draggable="false">
    <div class="flex justify-between items-center mb-4">
        <div>
            <h3 class="text-lg font-bold">{{ $character->name }}</h3>
            <p class="text-sm text-gray-500">{{ $character->species->name }} {{ $character->archetype->name }}, Level {{ $character->level }}</p>
        </div>
        <div>
            @if (Auth::user()->role == "player")
                @if (Auth::user()->character != null)
                    @if (Auth::user()->character->id == $character->id)
                        <h1 class="rounded-full bg-indigo-200 py-2 w-[7rem] text-xs font-bold text-indigo-700 text-center">ACTIVE</h1>
                    @endif
                @endif
            @else
                <h1 class="rounded-full bg-indigo-200 py-2 w-[7rem] text-xs font-bold text-indigo-500 text-center">{{ strtoupper($character->user->username) }}</h1>
            @endif
        </div>
    </div>
